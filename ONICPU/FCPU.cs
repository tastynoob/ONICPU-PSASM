...
namespace ONICPU
{
    ...
    public class FCPU : LogicGateBase, ILogicEventSender, ILogicNetworkConnection, ISaveLoadable, ISimEveryTick
    {
        ...
        [Serialize]
        private byte[] serializedContext;//JSON
        [Serialize]
        ...
        private bool DoCompileProgram()
        {
            ...
            else
            {
                ...
                executor.cpuState = FCPUState.HaltByError;
        ...
        private void InitExecutor()
        {
            ...
            executor.Init();
            bool isFromSerialize = executor.Restore(serializedContext);
            if (isFromSerialize) return;
            if (DoCompileProgram() && executor.cpuState == FCPUState.Looping)
                executor.Start();
        }
      ...
      new HashedString("LogicGateOutputSixteen"),
        ...
        {
            serializedContext = executor.Save();
        }
        ...
        private void UpdateStateControl(int new_value, int prev_value)
        {
            ...
            if (!showProgramEditor)
            {
                ...
                {
                    if (executor.cpuState != FCPUState.Looping && !programCompileErrorState)
                        executor.Start();
                }
                else
                {
                    if (executor.cpuState == FCPUState.Looping)
                        executor.Stop();
                }
            }
        ...
        public void OnShowProgramEditor()
        {
            if (!showProgramEditor)
            {
                ...
                if (FCPUEditorUI.ProgramEditorPanelPrefab)
                {
                    ...
                    {
                        if (executor.cpuState == FCPUState.Looping)
                            executor.cpuState = FCPUState.HaltByUser;
